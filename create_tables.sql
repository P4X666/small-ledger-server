...
CREATE TABLE `tasks` (
  ...
  `status` ENUM('待办', '进行中', '已完成') DEFAULT '待办' COMMENT '任务状态',
  ...
  `time_period` ENUM('周', '月', '年') DEFAULT '周' COMMENT '时间周期',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `transactions` (
  `id` INT PRIMARY KEY AUTO_INCREMENT COMMENT '唯一标识',
  `user_id` INT NOT NULL COMMENT '用户ID',
  `type` ENUM('收入', '支出') NOT NULL COMMENT '类型（收入/支出）',
...
CREATE TABLE `savings_goals` (
  ...
  `period` ENUM('月度', '季度', '半年', '年度') NOT NULL COMMENT '时间周期',
  `start_date` DATE NOT NULL COMMENT '开始日期',
  `end_date` DATE NOT NULL COMMENT '结束日期',
  `status` ENUM('进行中', '已完成', '已失败') DEFAULT '进行中' COMMENT '状态',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_status` (`status`),
